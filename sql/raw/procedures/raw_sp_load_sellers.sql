...
CREATE OR ALTER PROCEDURE raw.sp_load_sellers
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (
            @batch_id,    @job_run_id, @pipeline_id, 'raw.sp_load_sellers',
            @error_msg,   SYSUTCDATETIME(), ISNULL(@file_name, 'UNKNOWN'), 16, 'raw.sp_load_sellers', NULL
        );
    ...
    VALUES (
        @batch_id,      @job_run_id, @pipeline_id,
        'RAW',          'raw.sp_load_sellers', 'raw.sellers',
    ...
    BEGIN TRY
        ...
        WHERE batch_id = @batch_id AND sp_name = 'raw.sp_load_sellers';
